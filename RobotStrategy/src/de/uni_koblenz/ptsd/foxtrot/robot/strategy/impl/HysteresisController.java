...
/** Handles sticky target selection to avoid flip-flopping. */
final class HysteresisController {
    ...
    PlanCandidate apply(Target currentTarget, PlanCandidate chosen, long tickCounter, Function<Target, PlanCandidate> recompute) {
        ...
        if (cooldownActive || !bigEnoughGain) {
            PlanCandidate stick = recompute.apply(currentTarget);
    ...
    void onChosen(PlanCandidate chosen) {
        if (chosen != null) {
